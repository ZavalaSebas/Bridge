...
- **Own Cloudflare Worker as the IGDB backend** — `Bridge.Infra/igdb-proxy-worker/` + `BridgeIgdbProvider` give IGDB metadata with zero user configuration (credentials as Worker Secrets server-side, the same architecture Playnite uses). Playnite's public proxy and a user-configured IGDB key are fallbacks. See ADR-13.
