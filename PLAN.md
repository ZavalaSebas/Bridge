...
- **WPF-UI visual overhaul (Fase 7)** — `FluentWindow` with Mica backdrop, 3-zone title bar (search, view-mode toggles, overflow menu), 52px icon sidebar (Library / Statistics), themed List/Covers/Details views, and a full-width Statistics overlay
...
- **Own Cloudflare Worker as the IGDB backend** — `Bridge.Infra/igdb-proxy-worker/` + `BridgeIgdbProvider` give IGDB metadata with zero user configuration (credentials as Worker Secrets server-side, the same architecture Playnite uses). Playnite's public proxy and a user-configured IGDB key are fallbacks. The Worker also returns IGDB's real screenshots (mapped to `Game.Screenshots` at `t_1080p`), so Epic/manual games get the Details screenshot gallery like Steam games. See ADR-13.
...
- Full emulation subsystem: CRC/serial/partial-name ROM matching, multiple emulator profiles, scanner exclusions. (Bridge-managed RetroArch covers 15 systems today via `RomPlatformCatalog`; third-party emulators still need manual configuration.)
...
│                        #   ViewModels/ (MainViewModel, EmulatorSetupViewModel, EmulationSettingsViewModel, IgdbSettingsViewModel)
│                        #   Services/ (GameLauncher, RomScanner, RomPlatformCatalog, RetroArchService, ProcessTreeSnapshot, ProcessTreeExpander)
...
│                        #   Views: MainWindow (CompactInfoPanel detail/info panel inline —
...
└── Bridge.Tests/        # created — 204 tests, all passing (dotnet test Bridge.slnx)
...
| Microsoft.EntityFrameworkCore.Design | 10.0.10 | EF Core migration tooling (`dotnet ef migrations`) | Design-time only (`PrivateAssets=all`) — powers `Bridge.Storage/Migrations/`, see DEVELOPMENT.md "Schema Migrations" |
| dotnet-ef (global tool) | 10.0.11 | `dotnet ef migrations add ...` | Dev-only; paired with the Design package for schema migrations |
| SQLitePCLRaw.bundle_e_sqlite3 | 3.0.3 | Native SQLite binary (SourceGear.sqlite3 3.50.4) | Explicit pin in `Bridge.Storage` to fix GHSA-2m69-gcr7-jv3q (CVE-2025-6965) — the EF Core transitive `lib.e_sqlite3` 2.1.11 bundles vulnerable SQLite 3.49.1 with no patched 2.1.x release |
| CommunityToolkit.Mvvm | 8.4.2 | MVVM boilerplate (`ObservableObject`, `RelayCommand`) | Matches `DEVELOPMENT.md` MVVM conventions |
| Microsoft.Extensions.DependencyInjection | 10.0.10 | DI container | Matches `DEVELOPMENT.md` DI conventions |
...
| SharpCompress | 0.48.0 | 7z/zip extraction for the managed RetroArch install (`ArchiveFactory.WriteToDirectory` for solid 7z) | Pinned at 0.48.0 to fix CVE-2026-44788 / GHSA-6c8g-7p36-r338 (directory-traversal via directory entries in `WriteToDirectory`) — see the Managed Emulation section in DEVELOPMENT.md |
