...
### Phase 3 (Metadata, Emulation & Polish) — Metadata and Emulation done for MVP, Polish not started
Metadata download (IGDB) and emulator detection/ROM scanning both work end-to-end (see the Development Phases table for the caveats on each). Visual polish (Fase 7), packaging (Fase 8), and consolidation (Fase 9) have not started — see [Development Phases](#development-phases) for the full per-fase breakdown and every known gap, not just this summary.
...
| Fase 5 — Metadata | Download name/description/images, cache results, respect existing values | **Unblocked and built** — user confirmed IGDB ([ADR-10](ARCHITECTURE.md#adr-10-igdb-as-the-sole-text-metadata-source)). `Bridge.Metadata` (a real separate project, not folded into `Bridge` — see the note above about not repeating that deviation) has a working `IgdbMetadataProvider`, verified against 9 tests using a fake HTTP handler (OAuth flow, request format, response mapping, error paths) since the assistant has no real IGDB credentials to test the live call with. `MainViewModel.DownloadMetadataCommand` applies Description/ReleaseDate/CoverImage/Genres to the selected game and saves. **Not done**: caching results (currently re-downloads every time), `SkipExistingValues` (always overwrites), Developers/Publishers mapping, local image download (cover stored as a raw URL, not cached to disk) |
