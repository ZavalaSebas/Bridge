...
└── Bridge.Tests/        # created — 47 tests, all passing (dotnet test Bridge.slnx)
```
...
| Fase 4 — Basic statistics | Totals, installed/not installed, favorites/hidden, total playtime, top played | Done for MVP — `Bridge/Statistics/LibraryStatistics.cs` computes everything listed on the fly from the current `Games` list (no persisted stats entity — the same on-the-fly approach Playnite takes, §28.5). Verified against known test data (exact expected counts/totals). Shown in `MainWindow`'s status bar, recomputed after every Add/Delete/Save/GameStopped. `TopPlayed` is computed but not yet displayed anywhere in the UI |
