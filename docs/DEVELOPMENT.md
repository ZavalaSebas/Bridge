...
- **Covers** (`ViewMode.Covers`): `ListBox` whose `ItemsPanel` is `controls:CenteringWrapPanel` (a custom wrap panel that also centers each row) with 200×300px cards (base size, driven through the `ZoomToSize` converter) with hover intent (scale 1.03x + drop shadow + overlay fade, 120ms `CubicEase EaseOut`). The panel invalidates measure on `SizeChanged` so covers re-wrap when the views column expands from the List-view width (360px) to the full Covers layout. When grouping is on, `GroupStyle.Panel` uses the same `CenteringWrapPanel` (see Grouping above). Game Info / **I** opens the compact 320px panel when Settings → Appearance → **Covers detail layout** is Compact (default); **Full details** docks the same Details pane as List view at a 50/50 split (`CoversDetailLayoutSettingsStore`, `covers-detail-layout.txt`). List view always uses the large details pane.
...
Detail panel (right): `CoverImage` 200px + title/scores/checkboxes + Play (uses `SystemAccentColorPrimaryBrush`, `#007ACC`, `CornerRadius="2"`) / More (ContextMenu with Save/Download Metadata/Delete) / Edit buttons. Below: Details (label/value pairs in accent blue) + Description (read-only here — editable via the Edit window, `GameEditWindow`). Background image (`SelectedGame.BackgroundImage`) fills the panel at 60% opacity with dark overlay. Overview / Images / Achievements sit in `DetailContentDock` inside a `StackPanel` whose `Width` binds to `DetailScrollViewer.ViewportWidth`, so the tab strip stretches to the pane instead of jumping to content width.

Below the Details/Description row, when the selected game has screenshots (`SelectedGame.Screenshots.Count > 0`), the **screenshot gallery** renders (`Bridge/Controls/ScreenshotGallery.xaml(.cs)`): a large main image floating over a frosted backdrop of the same screenshot, with previous/next arrows, a counter (`1 / N`), a drag-to-scroll thumbnail strip (no scrollbar — `Preview*` mouse events pan the `ScrollViewer`), and click-to-expand into a full-window dark overlay with its own thumbnail strip, arrows and counter. The carousel auto-advances every 4s (paused while the mouse is over the main image or the overlay is open; only runs with 2+ screenshots). The main image sizes to 72% height / 82% width of its container via `Converters/FractionConverter`, keeping the "image inside image" look proportional at any window size. The gallery appears in the **Table** view (`ViewMode.Table`) and as a compact strip in the **Covers** info panel (`ViewMode.Covers`, `CompactMode="True"`). Screenshots come from either provider — Steam (`path_full`) or the IGDB Worker (`t_1080p`, non-Steam games) — they all land in the same `Game.Screenshots` JSON column. A `CompactMode` property (used by the covers info panel) hides the header and main image, shows only a smaller thumbnail strip (110×62 tiles), and opens the fullscreen overlay straight from a thumbnail tap; auto-advance never runs in that mode. Set `CompactMode` **before** `ItemsSource` in XAML (or call `ApplyCompactMode` on `Loaded`): if URLs bind first, the gallery measures the 320px main image and the compact strip overflows. Compact mode also skips loading `MainImage`/`BackdropImage` (thumbs + fullscreen only) and collapses the control when there are no screenshots.

The **covers info panel** (`CompactInfoPanel`) shows its own compact layout: the hero art fades into the panel background via a strong vertical gradient (ending in the panel's `#CC1B2132` so there's no hard edge), a larger game title (22px bold, drop shadow) with the favorite star inline after it (`InlineUIContainer` in the title `TextBlock`, so the star follows the title's wrapping lines and never gets cut), the Play/More/Edit row sitting on the hero under the title, and a theme-colored square close button in the top-right. Compact hero (`CompactHeroImage`) uses `DecodeSize="Hero"` **before** `SourceUrl`, `Stretch="UniformToFill"`, `Height="170"`, and `ClipToBounds` inside the 170px border — it must **not** use `CoverByWidth` (that full-bleed sizing is for the Details hero only and overflows the compact frame). `FadeImage` reloads when `DecodeSize` changes, when the control becomes visible, and if the current frame is empty after `Unloaded` unsubscribed a pending decode (collapsed compact panel). The link list wraps to two columns (`WrapPanel` + per-item `MaxWidth`). The Play button is the same animated Play/Stop template as the Details hero — there its `IsRunning` DataTriggers bind to `SelectedGame.IsRunning` because the panel's DataContext is the `MainViewModel`, not the game.

The hero cross-fade (`FadeImage.xaml.cs`, `CoverByWidth` on the Details hero) animates the outgoing tall frame's bottom fade in sync with the incoming short frame (`AnimateOutgoingTallFade` + animated `ApplyShortFrameFadeTo`), so switching from a tall background (e.g. a 16:9 screenshot) to a shorter one no longer snaps the blur edge when opacity reaches zero. `DecodeSize` is part of the `RemoteImageCache` key; changing it unsubscribes the old bucket and reloads. Hosts that set `SourceUrl` before `DecodeSize` in XAML used to stay subscribed to Native until Details opened.
...
All styling is defined in `Bridge/Styles/Theme.xaml` (indigo-tinted dark palette #151A28/#1B2132/#232B40/#2C3550, font family via `Bridge.FontFamily` token — Inter Variable (`InterVariable.ttf` 879KB) + Outfit Variable (`OutfitVariable.ttf` 110KB) embedded in `Bridge/Fonts/` plus system fallbacks Segoe UI / Georgia / Consolas — spacing and corner radius tokens, motion durations). The dictionary overrides WPF-UI's semantic tokens in Color+Brush pairs — see ADR-3 in ARCHITECTURE.md for the full decision record. `Bridge.FontFamily` is a `DynamicResource` (all windows use `{DynamicResource Bridge.FontFamily}`) so `FontManager.Apply()` (`Bridge/Services/FontManager.cs`, persisted to `config/font.txt` via `FontSettingsStore`) can switch the typeface at runtime without restart, with `ThemeManager.RefreshWindow()` re-applying styles.
...
| `font.txt` | `FontSettingsStore` / `FontManager` | Inter | Curated font family (Inter / Segoe UI / Outfit / Georgia / Consolas), `DynamicResource` switch |
...
| `covers-detail-layout.txt` | `CoversDetailLayoutSettingsStore` | Compact | Covers: compact info panel vs full details at half window |
