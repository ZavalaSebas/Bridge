> **Archivado en 1.0 � 2026-08-24:** Este documento es hist�rico. Todas las fases y m�dulos descritos (Fases 0�9, �16��26) est�n implementados en Bridge 1.0 � ver docs/PLAN.md y docs/CHANGELOG.md. No usar como backlog activo.
...
LinkItem. No existe MetadataImage — tanto GameMetadata.Icon como CoverImage y
BackgroundImage usan la misma clase MetadataFile. No existe una clase GameStatistics
dedicada — las estadisticas se calculan en tiempo real dentro de StatisticsViewModel
(clases internas GameStats/BaseStatInfo). "GameFilter" corresponde a FilterPresetSettings
(el filtro persistido se llama FilterPreset). "LibrarySource" no es una clase — la
...
21. DECISIONES IMPORTANTES DEL DISEÑO
...
La reescritura planteada debe conservar la esencia funcional, pero simplificar drásticamente la arquitectura:
- .NET 10,
- WPF primero,
- WPF UI despues,
- SQLite o LiteDB,
...
- debe apoyar decisiones de diseño durante la reescritura,
...
Esta seccion NO propone diseño para Bridge — es referencia pura de qué hace Playnite
hoy. La adaptacion a las entidades reales de Bridge.Core es un paso aparte.

--------------------------------------------------------------------------------
28.1 MODELO DE DATOS — CAMPOS REALES

Game — PlayniteSDK/Models/Game.cs (lineas 188-2462)
...
NO secuenciales (hay huecos, ej. saltan 8, 13-15, 32, 34-35, 46-49, 52 — reflejan
campos eliminados historicamente). No asumir que la numeracion es secuencial si se
porta este enum.

GameMetadata — PlayniteSDK/Models/GameMetadata.cs (lineas 276-465)
...
MetadataProperty (jerarquia, mismo archivo, lineas 98-271):
  ...
  MetadataIdProperty     : Guid Id     — referencia un objeto existente por id
  MetadataNameProperty   : string Name — resuelto/creado por nombre (match case-insensitive)
  MetadataSpecProperty   : string Id   — "specification id" (ej. slugs de IGDB para
                                          Platform/Region), NO tiene resolucion
                                          generica en ItemCollection, se usa a mano
                                          en sitios puntuales (ej. escaneo de emuladores)

Link (NO "LinkItem") — PlayniteSDK/Models/Link.cs
  string Name
  string Url

MetadataFile (NO existe clase separada "MetadataImage") — GameMetadata.cs lineas 13-93
  string FileName
  byte[] Content
...
Emulator / EmulatorProfile — PlayniteSDK/Models/Emulator.cs
  ...
  class Emulator : DatabaseObject
    ...
    GetProfile(string profileId)  — busca primero en CustomProfiles, luego Builtin
...
GameScannerConfig — PlayniteSDK/Models/GameScannerConfig.cs (lineas 36-334)
  Guid EmulatorId
...
Name heredados, mas un sentinel estatico Empty):
  Genre           — sin campos extra
  Company (base), Developer : Company, Publisher : Company
                  — Developer/Publisher NO agregan campos, son solo tipos marcadores
                    para MetadataProperty; la DB real guarda todo en una unica
                    coleccion compartida Companies
  Category / Tag / Series          — sin campos extra
  AgeRating                        — sin campos extra (AgeRatingOrg es un enum
                                      separado: PEGI, ESRB — no un campo de AgeRating)
  Region            — agrega string SpecificationId
  Platform          — agrega string SpecificationId, string Icon, string Cover,
                      string Background (imagenes por defecto del sistema)
  CompletionStatus  — sin campos extra (el estado "jugado"/"default" es externo,
                      vive en CompletionStatusSettings — ver S28.2)
  GameFeature       — sin campos extra (entidad separada de Category/Tag)
  GameSource        — sin campos extra; ES la entidad normalizada de "fuente/origen"
...
esto. Es directamente dos campos crudos en Game:
  string GameId   — id nativo de la tienda (ej. Steam AppID)
  Guid PluginId   — id del LibraryPlugin responsable (Guid.Empty si es manual;
                    Game.IsCustomGame es literalmente PluginId == Guid.Empty)
GameSource (arriba) es un tag de origen aparte, editable por el usuario — mas
parecido a una categorizacion manual que a la identidad de plugin.

FilterPreset — PlayniteSDK/Models/FilterPreset.cs
...
GameStatistics: NO existe como clase de modelo dedicada. Confirmado — no hay ninguna
...
--------------------------------------------------------------------------------
28.2 GameDatabase — PERSISTENCIA Y FLUJO DE IMPORTACION REAL
...
— Games, Platforms, Emulators, Genres, Companies (Developer+Publisher compartido),
...
ItemCollection<TItem> — CRUD generico:
  ...
  void Add(TItem) / Add(IEnumerable<TItem>)   — throw si el Id ya existe
  bool Remove(Guid id) / Remove(TItem) / Remove(IEnumerable<TItem>)
  void Update(TItem) / Update(IEnumerable<TItem>)
  BeginBufferUpdate() / EndBufferUpdate() / BufferedUpdate()  — agrupa eventos
...
ImportGames(LibraryPlugin library, CancellationToken, PlaytimeImportMode) —
algoritmo real (GameDatabase.cs lineas 1205-1338):
  ...
  3. Rama A — library.Properties?.HasCustomizedGameImport == true: llama
     ...
     Game devuelto.
  4. Rama B — flujo estandar, por cada GameMetadata newGame que devuelve
     library.GetGames(...):
     ...
     - Clave de dedup: Games.FirstOrDefault(a => a.GameId == newGame.GameId &&
       a.PluginId == library.Id) — match exacto sobre el par (GameId, PluginId),
     ...
     - Juego EXISTENTE (existingGame != null) — solo estos campos se tocan, y
       solo condicionalmente (el resto — Name, Description, imagenes, generos,
       etc. — NUNCA se toca en un re-import, solo lo toca MetadataDownloader):
...
ImportGame / GameInfoToGame (lineas 1081-1203) — conversion GameMetadata -> Game:
...
PERSISTENCIA EN DISCO — correccion importante sobre lo asumido en S4:
...
GameDatabase.cs lineas 126-248):
  ...
  - Cada una de esas rutas de coleccion es en realidad el nombre base de un
    ...
    — o sea la ruta "games" se convierte en games.db en disco (una base LiteDB v4
  ...
  - database.json (el de nivel superior, no el de cada coleccion) guarda solo
    DatabaseSettings { Version } — gate de version de esquema (NewFormatVersion
...
Almacenamiento de archivos — carpeta files/, AddFile/GetFileStoragePath/
GetFileAsImage (lineas 787-1032):
  ...
  - AddFile(string path, Guid parentId, bool isImage, CancellationToken):
    ...
    - El dbPath devuelto siempre es una ruta RELATIVA con forma
      "<guid-parentId>/<nombreArchivo>" (no un hash — un Guid nuevo por archivo,
      excepto en el caso "ya esta en el directorio destino" donde se conserva el
      nombre original).
  - GetFullFilePath(dbPath) -> Path.Combine(FilesDirectoryPath, dbPath) — resuelve
  ...
  - Constantes: MaximumRecommendedIconSize = 0.1, MaximumRecommendedCoverSize = 1,
    MaximumRecommendedBackgroundSize = 4 — solo orientativas (unidades: MB).

--------------------------------------------------------------------------------
28.3 MetadataDownloader — FLUJO REAL DE RESOLUCION DE METADATOS

Playnite/Metadata/MetadataDownloader.cs, .../MetadataDownloaderSettings.cs,
PlayniteSDK/Plugins/MetadataPlugin.cs

ProcessField — algoritmo exacto por campo (lineas 136-309):
  ...
  2. Itera fieldSettings.Sources EN EL ORDEN CONFIGURADO (el orden literal que el
     usuario definio en Ajustes > Metadatos — la primera fuente que da datos
     validos gana):
     ...
     - Si los datos de esta fuente ya se pidieron en esta corrida
       ...
       fuente (el cache es por FUENTE, no por campo — una sola descarga de store
       se reusa entre todos los campos).
     - Si source != Guid.Empty, busca el MetadataPlugin por id y chequea
       downloader.SupportedFields?.Contains(gameField) — salta la fuente entera
       si no puede proveer este campo.
     - Descarga: source == Guid.Empty -> ProcessStoreDownload(game) (llama
       LibraryMetadataProvider.GetMetadata(game) del LibraryPlugin dueño),
...
SkipExistingValues — patron literal repetido por campo en DownloadMetadataAsync
(lineas 311-636): para cada campo el chequeo es
  if (!settings.SkipExistingValues || (settings.SkipExistingValues && <campo-vacio>))
— o sea siempre se procesa si SkipExistingValues == false; si es true, solo se
...
Esto confirma y completa la lista de S7 — CommunityScore y CriticScore estan
ambos presentes (campos separados), e InstallSize esta presente con un guard
especial: isInstalledAndHasValue = game.IsInstalled && game.InstallSize != null —
si es true, la descarga de InstallSize se salta por completo sin importar
SkipExistingValues, porque un tamaño escaneado localmente en un juego instalado
se considera mas confiable que el tamaño que reporta la fuente de metadatos.
NO estan en el loop de resolucion (o sea NUNCA los llena MetadataDownloader):
UserScore, Notes, Version, Manual, los scripts — son campos manuales/de usuario
exclusivamente.

Guardado de imagenes — mismo mecanismo de archivos que S28.2, pero con logica
distinta segun el campo:
  - BackgroundImage tiene logica propia distinta de Icon/Cover: controlada por
    ...
    directo a la URL CRUDA (gameData.BackgroundImage.Path) — o sea los fondos
  ...
  - CoverImage e Icon: siempre se materializan de inmediato via
    database.AddFile(gameData.CoverImage/Icon, game.Id, true, cancelToken) — sin
  ...
  - Al terminar de procesar todos los campos: solo escribe de vuelta a la DB
    ...
    existiendo en la DB (re-chequeado por id) — marca game.Modified = DateTime.Now
    primero.
  - Opera sobre un CLON: game = database.Games[games[i].Id]?.GetClone() — nunca
    muta la instancia viva de la DB directamente, para no bloquear la edicion de
    otros juegos durante una descarga por lote.

--------------------------------------------------------------------------------
28.4 EMULACION — DETECCION, ESCANEO Y MATCHING REALES

Playnite/Emulators/Scanner.cs, .../EmulationDatabase.cs, .../DatModels.cs

Deteccion de emuladores — EmulatorScanner.SearchForEmulators (lineas 55-146):
  ...
  2. Por cada archivo x cada EmulatorDefinition x cada EmulatorDefinitionProfile
     ...
     - detectionStr = defProfile.InstallationFile ?? defProfile.StartupExecutable
       — un PATRON REGEX, compilado al vuelo por cada par archivo/perfil (new
  ...
  3. Devuelve la lista dedupeada; esto NO escribe nada en disco — solo propone
...
Escaneo de ROMs — GameScanner.Scan (lineas 148-1300):
  ...
  1. Resuelve Emulator y perfil (CustomEmulatorProfile si EmulatorProfileId
     empieza con "#custom_", BuiltInEmulatorProfile si empieza con "#builtin_" —
     si no, lanza "not supported"). Lanza excepcion si EmulatorId/
     EmulatorProfileId no resuelven.
  2. importedFiles = database.GetImportedRomFiles(emulator.InstallDir) — arma el
  ...
  4. dirToScan = PlaynitePaths.ExpandVariables(scanner.Directory,
     emulator.InstallDir, true) — expande tokens como el directorio del emulador
     relativo al Directory configurado del scanner.
  5. Parsea scanner.ExcludedFiles/ExcludedDirectories via ParseExclusions — un
  ...
  7. ScanDirectoryBase — el recorrido recursivo real:
     ...
     - El escaneo de CRC se salta condicionalmente: si scanner.ExcludeOnlineFiles
       y el archivo no esta disponible localmente (IsFileDataAvailable — chequea
     ...
     - Lookup en DB (LookupGameInDb) prueba, EN ESTE ORDEN EXACTO, parando en el
       primer hit: (1) match por CRC — si el archivo es un tipo de archivo
       ...
       (db.GetByRomNamePartial — "principalmente para juegos XBLA con nombres de
       archivo raros"). (4) match de Serial usando el nombre de archivo sin
       extension (db.GetBySerial — "para casos raros donde el nombre del archivo
...
EmulationDatabase — formato confirmado SQLITE, un .db por nombre de base de datos
de plataforma (Playnite/Emulators/EmulationDatabase.cs):
  ...
  (Replace("'", "''")) — SQL crudo armado por concatenacion, NO queries
  parametrizadas.
...
--------------------------------------------------------------------------------
28.5 VISTAS Y VIEWMODELS — SUPERFICIE REAL DE CAMPOS Y COMANDOS

GameDetailsViewModel — Playnite.DesktopApp/ViewModels/GameDetailsViewModel.cs
El objeto bindable central es GamesCollectionViewEntry Game (NO Game directamente
— ver mas abajo). Bools de estado/computados: IsRunning, IsInstalling,
...
settings.FilterSettings.<Campo> y fuerza settings.FilterPanelVisible = true —
...
StatisticsViewModel — Playnite.DesktopApp/ViewModels/StatisticsViewModel.cs
Lista de dimensiones de filtro List<FilterSection> Filters — un set fijo de
...
— los caches de ids-en-uso que GameDatabase mantiene, ver S28.2).

Las estadisticas calculadas viven en clases planas anidadas (esto es lo mas
parecido a "GameStatistics" que existe — no hay clase de nivel superior separada):
  class GameStats
...
metodo FillData(bool filtered) — una sobre toda la biblioteca (sin ocultos salvo
...
PART_ButtonPlay, PART_ButtonInfo), no una clase de datos — no tiene lista de
...
GamesCollectionViewEntry (Playnite/GamesCollectionViewEntry.cs) — envuelve un
...
refrescada en Game.PropertyChanged):
  ...
  - Accesores de objetos de imagen (cada uno resuelve via ImageSourceManager,
    cacheado vs no-cacheado, con BitmapLoadProperties por modo de vista —
...
1. La persistencia es LiteDB, no JSON por entidad (ver S28.2) — la correccion mas
   importante de esta seccion.
2. "LinkItem" no existe — la clase se llama Link.
3. No existe "MetadataImage" — Icon, CoverImage y BackgroundImage de GameMetadata
   usan todos la misma clase MetadataFile.
4. No existe una clase "GameStatistics" dedicada — se calcula enteramente dentro
   de StatisticsViewModel en clases anidadas locales (GameStats, BaseStatInfo).
5. Developer/Publisher son subclases casi vestigiales de Company — la coleccion
...
6. La "base de datos" de emulacion SI es SQLite de verdad (via el wrapper
   micro-ORM SqlNado, tabla DatGame) — coincide con lo asumido — pero se
...
7. Los valores del enum GameField NO son secuenciales, tienen huecos (ej. saltan
   de 4 a 9, se saltean 8, 13, 14, 15, 32, 34, 35, 46-49, 52) — claramente
   ...
   serializado en datos persistentes — se usa sobre todo en diffing en memoria —
...
8. El flujo ImportGames estandar (sin HasCustomizedGameImport) re-sincroniza MUY
   ...
   de completado — Name, imagenes, generos, descripcion, etc. NUNCA se
...
9. Los fondos (BackgroundImage) pueden quedar como URLs http sin materializar en
   ...
   PlayniteSettings.DownloadBackgroundsImmediately == false — por eso
   ...
   perezoso — siempre se materializan.

--------------------------------------------------------------------------------
28.7 (continua en 28.8) — LOS HUECOS DE LA PRIMERA PASADA (GameAction, ejecucion,
...
enum TrackingMode (lineas 15-42), valores:
  Default = 0         — "Playnite intenta usar el mejor modo automaticamente."
  Process = 1         — se trackea el proceso original y todos sus hijos
  Directory = 2       — se trackea cualquier proceso que corra desde una carpeta dada
  OriginalProcess = 3 — se trackea SOLO el proceso originalmente lanzado
  ProcessName = 4     — se trackea cualquier proceso por nombre de proceso dado
...
Uso de campos por tipo:
  ...
  Emulator: EmulatorId, EmulatorProfileId, OverrideDefaultArgs, Arguments (solo
            ...
            del perfil si no). Path/WorkingDir NO se usan — el ejecutable/workdir
...
EmulatorProfile SelectedEmulatorProfile y string SelectedRomPath — es la forma
resuelta y lista-para-lanzar que se arma en el momento de jugar a partir de un
GameAction crudo de tipo Emulator.

GameRom — PlayniteSDK/Models/GameRom.cs (lineas 1-103)
class GameRom : ObservableObject, IEquatable<GameRom> — solo dos campos de datos:
  string Name
  string Path
Nada mas — sin tamaño, sin checksum, sin lista de extensiones. (El checksum/CRC
...
Jerarquia de controladores:
  - SDK: Playnite.SDK.Plugins.ControllerBase (PlayniteSDK/Plugins/Actions.cs
    ...
    123-255) — cada una con metodos abstractos Play/Install/Uninstall y helpers
  ...
  - Playnite.SDK.Plugins.AutomaticPlayController : PlayController (sellada,
    lineas 29-86) — un controlador de conveniencia para plugins con
    ...
    propio Play() no hace nada — Playnite detecta este tipo y arma internamente
  ...
  - Orquestacion: Playnite.Controllers.GameControllerFactory
    (Playnite/Controllers/GameControllerFactory.cs lineas 15-206) — mantiene
  ...
  - Driver de mas alto nivel: Playnite.GamesEditor (Playnite/GamesEditor.cs
    ...
    (Playnite.DesktopApp/DesktopGamesEditor.cs, solo 76 lineas — apenas agrega
    ...
    FullscreenPlayController separados — ambas apps usan exactamente el mismo
...
PLAY — paso a paso (GamesEditor.PlayGame, Playnite/GamesEditor.cs lineas 189-422):
  ...
  5. Resolucion de accion: si Item1.Count + Item2.Count > 1 ->
     ...
     Fullscreen). Si no, se usa directamente el unico candidato disponible — no
  ...
  10. Dispara controllers.InvokeOnStarting(this, startingArgs) (un
      ...
      PlayniteSDK/Events/ApplicationEvents.cs lineas 128-149) — las extensiones
      pueden setear startingArgs.CancelStartup = true para abortar aca.
  11. Si el plugin dueño del juego tiene un timer pendiente de apagado de
      cliente, se cancela.
...
MECANICA DE LANZAMIENTO — GenericPlayController.Start(GameAction, bool asyncExec,
OnGameStartingEventArgs) (GenericGameController.cs lineas 481-668):
  ...
  3. Si no, lanza el proceso:
     - Type == File -> Process proc = ProcessStarter.StartProcess(action.Path,
       action.Arguments, action.WorkingDir) — un wrapper fino de
       Process.Start(new ProcessStartInfo(...)) (Playnite/Common/
       ProcessStarter.cs lineas 95-121).
     - Type == URL -> Process proc = ProcessStarter.StartUrl(action.Path) —
  ...
  4. Caso especial UWP: si action.Path == "explorer.exe" y Arguments matchea
     ...
     explorer.exe lanzado (no es el juego) — en vez de eso resuelve el
  ...
  5. Si no, despacho de tracking segun action.TrackingMode:
     - Default: para acciones no-URL (y sin el caso especial UWP), si
       ...
       acciones URL bajo Default, no hay proceso que trackear — en vez de eso
       monitorea gameClone.InstallDirectory via MonitorDirectory.
     - Process: igual que el caso de proceso de Default —
       MonitorProcessTree(proc.Id).
     - OriginalProcess: MonitorProcess(proc) — solo el PID exacto lanzado, sin
       hijos.
     - Directory: MonitorDirectory(action.TrackingPath ??
       gameClone.InstallDirectory).
     - ProcessName: MonitorProcessName(action.TrackingPath) — matchea por
       Process.ProcessName.
     - Para los casos Directory/ProcessName, el proceso LANZADO nunca se asume
       que es el juego — StartTracking recibe una funcion startupCheck que
  ...
  6. Las acciones tipo Emulator se rechazan explicitamente aca (throw
     "Cannot start emulator using this configuration.") — van por el flujo
     StartEmulator separado.

LANZAMIENTO DE EMULADOR — GenericPlayController.StartEmulator(EmulationPlayAction,
bool asyncExec, OnGameStartingEventArgs) (GenericGameController.cs lineas 72-219):
  ...
  2. Determina currentEmuProfile — un CustomEmulatorProfile o
  ...
  6. StartEmulatorProcess(path, args, workDir, emulatorDir, romPath, asyncExec,
     ...
     - Corre el PreScript propio del emulador (del perfil) antes de lanzar, el
       ...
       el tracking detecta que termino — todo via ExecuteEmulatorScript(...)
...
INSTALL / UNINSTALL — NO hay un tipo de accion separado. Confirmado por lectura
...
Emulator — un juego agregado manualmente, o un juego de biblioteca cuyo plugin no
...
SCRIPTS Pre/Post/GameStarted — puntos de invocacion y motor:
...
Dictionary<string,object> vars) (GamesEditor.cs lineas 1617-1680) — arma
...
— se usa solo para el titulo del dialogo de error.

Orden respecto al lanzamiento (confirmado en PlayGame y los handlers
Controllers_Started/Controllers_Stopped):
  1. PreScript GLOBAL (AppSettings.PreScript, solo si game.UseGlobalPreScript) —
     antes de invocar el controlador de play (PlayGame linea 358).
  2. PreScript POR JUEGO (game.PreScript, siempre se intenta) — justo despues,
  ...
  4. En el evento Started del controlador -> Controllers_Started
     ...
     game.UseGlobalGameStartedScript) — ambos disparan solo despues de que el
...
ExecuteEmulatorScript (GenericGameController.cs lineas 240, 263, 273, etc.) —
...
--------------------------------------------------------------------------------
28.10 TRACKING DE PLAYTIME — EL MECANISMO REAL DE MONITOREO DE PROCESOS

Clases de monitoreo (Playnite/Common/ProcessMonitor.cs, archivo completo,
lineas 1-197):
  - MonitorProcess(Process process) — IsProcessRunning() => !process.HasExited.
    Trackea el handle de Process EXACTO lanzado (modo OriginalProcess).
  - MonitorProcessTree(int originalId) — mantiene una List<int> relatedIds
    ...
    relatedIds.Count > 0.
  - MonitorDirectory(string directory) — resuelve la ruta final via
  ...
  - MonitorProcessName(string processName) (declarada fuera del namespace
    Playnite.Common, mismo archivo lineas 168-197) — escanea todos los procesos
    buscando process.ProcessName == ProcessName, devuelve su PID.
  - MonitorProcessNames(string directory) (plural, lineas 67-117) — un matcher
    ...
    *.exe bajo el directorio (recursivo) y matchea procesos corriendo por
    ...
    se conecta a TrackingMode.ProcessName en GenericPlayController — esa usa la
    MonitorProcessName singular por nombre de proceso literal desde
    TrackingPath.)

DETECCION DE INICIO — NO siempre confia en lo que devuelve Process.Start:
  ...
  existe) explicitamente NO se confia — en vez de eso StartTracking(...) recibe
...
DETECCION DE PARADA, Y EL LOOP DE TRACKING —
GenericPlayController.StartTracking (GenericGameController.cs lineas 670-775):
  ...
  4. Fase de EJECUCION: loop infinito —
     - trackingWatch.Restart(), llama trackingAction() (el predicado
       ...
       loop — ESTA ES LA DETECCION REAL DE "EL JUEGO SE DETUVO": polling, NO un
     ...
     - await Task.Delay(trackingFrequency, ...); despues
       ...
       (continue) sin sumar a playTimeMs — esto compensa que el sistema se haya
       suspendido para que el playtime no se corrompa.
     - Si no, playTimeMs += (ulong)trackingWatch.ElapsedMilliseconds — ESTE ES
...
seguridad ante crash):
  ...
  args.SessionLength, el valor derivado de playTimeMs de StartTracking) — pero
  ...
  desde el timestamp de reloj de pared y se suma a dbGame.Playtime — un camino
...
PUNTOS DE ACTUALIZACION de Playtime/LastActivity/PlayCount:
  - GamesEditor.UpdateGameState(Guid id, bool? installed, bool? running, bool?
    ...
    y game.PlayCount += 1 — PLAYCOUNT SE INCREMENTA AL INICIAR, NO AL PARAR.
  ...
  - Controllers_Stopped (GamesEditor.cs lineas 1416-1561): dbGame.IsRunning =
    ...
    despues Database.Games.Update(dbGame) — AQUI ES DONDE PLAYTIME (ulong,
...
sesion. Dos datos relevantes:
  1. El constructor de GamesCollection (Playnite/Database/Collections/
     ...
     a false en silencio — sin dialogo, sin recuperacion de tiempo transcurrido,
  ...
  2. PlaynitePaths.SafeStartupFlagFile ("safestart.flag") NO tiene relacion con
     sesiones de juego — es un guardian anti-crash-loop del propio Playnite: se
     ...
     la pena marcarlo en "sorprendentes" — suena relacionado a
...
al cargar. Evidencia:
  - La logica de merge de GameDatabase.ImportGames (GameDatabase.cs lineas
    ...
    ... } } — o sea, IsInstalled solo se sobreescribe durante un RE-ESCANEO DE
    ...
    instalados) — el nucleo de Playnite NUNCA hace
...
GameDetailsViewModel.cs lineas 306-307, 349-350, 514-534) — estos comandos NO
"chequean" ni "verifican" nada a pesar del nombre. Son literalmente identicos:
ambos muestran un dialogo de confirmacion Si/No ("¿Cancelar el monitoreo de
instalacion/ejecucion?") y, si es Si, llaman editor.CancelGameMonitoring
(game.Game) — que fuerza la parada del controlador de install/uninstall/play y
...
instalacion" — hallazgo sorprendente respecto de lo que sugiere el nombre.

--------------------------------------------------------------------------------
28.12 PlayniteSettings — MODELO DE CONFIGURACION GLOBAL

Archivo: Playnite/Settings/PlayniteSettings.cs (2775 lineas), clase
PlayniteSettings : ObservableObject (linea 261).

PERSISTENCIA — Archivos JSON planos, NO LiteDB, via Newtonsoft.Json
...
Tres archivos JSON separados (rutas en Playnite/Settings/PlaynitePaths.cs):
  - config.json (ConfigFileName) — el objeto PlayniteSettings principal.
  - fullscreenConfig.json (FullscreenConfigFileName) — un objeto
    FullscreenSettings separado, se carga/guarda independiente y solo se
    cuelga de settings.Fullscreen.
  - windowPositions.json (WindowPositionsFileName) — objeto WindowPositions
    separado.
...
CompletionStatusSettings (ver abajo) NO se guarda en config.json — se guarda
...
(confirmado por grep directo del archivo):
  ...
  FilterSettings FilterSettings (linea 914) — el estado de filtro ACTIVO/
    ultimo-usado (distinto del modelo FilterPresetSettings del SDK usado para
    presets guardados)
  ViewSettings ViewSettings (linea 928) — entre otras cosas,
    ListViewColumsOrder (List<GameField>) y ListViewColumns (un
    ListViewColumnsProperties con flags Visible por campo)
  MetadataDownloaderSettings MetadataSettings (linea 1448) — objetos
  ...
  AutoClientShutdownSettings ClientAutoShutdown (linea 1551) — ShutdownClients
    (bool), MinimalSessionTime, GraceTimeout, ShutdownPlugins (set de ids de
    plugin) — usado en la logica de auto-apagado de cliente tras cerrar el
    juego en Controllers_Stopped
  WindowPositions WindowPositions (linea 2248) — archivo separado
  FullscreenSettings Fullscreen (linea 2254) — archivo separado; incluye al
...
CompletionStatusSettings — campos exactos (CompletionStatusesCollection.cs
lineas 12-18):
...
biblioteca, nombre, año de lanzamiento, bucket de playtime, bucket de tamaño de
...
features) — TODOS los chequeos a nivel de campo se combinan con AND (un juego
...
Genre/Platform/Publisher/Developer/Category/Tag/Feature/Series/Region/AgeRating:
  ...
  - Si el filtro tiene Ids seteado:
    ...
    filter.Ids.Any(gameHasItemWithIdMatch) — O SEA: una sola lista
    ...
    (gamePropertyIds == null || gamePropertyIds.Count == 0) — asi aparece
...
IsFilterMatchingSingle (lineas 278-301) en vez de eso — siempre semantica
...
seleccionados" seria vacuamente falso salvo el caso de no-match — el codigo ni
...
enumFilter.Values.Contains(enumFieldValue) — chequeo OR plano de "el bucket
...
propiedades enum calculadas/agrupadas en Game, NO campos guardados crudos —
...
--------------------------------------------------------------------------------
28.14 EDICION MULTIPLE (GameTools) — ALGORITMO REAL
...
GameTools.GetMultiGameEditObject(IEnumerable<Game> games) (lineas 28-237) — el
algoritmo completo:
  ...
  3. CAMPOS ESCALARES (Name, SortingName, ReleaseDate, Description, Notes,
     ...
     games.All(a => a.Campo == valorPrimero) — o sea, la propiedad del objeto
  ...
  4. CAMPOS DE LISTA DE IDS MULTI-VALOR (Genre, Developer, Publisher, Category,
     ...
     valores presentes en la seleccion" (distinct/union) — patron de UI de
...
juegos seleccionados" — esa logica (presumiblemente diffear el MultiEditGame
...
DesktopGamesEditor.EditGames) que NO se abrio en esta pasada — marcar como
...
(Playnite/API/NotificationsAPI.cs, archivo completo, lineas 1-112):
  ...
  - Add(NotificationMessage) / Add(string id, string text, NotificationType
    type) / Remove(string id) / RemoveAll() — las cuatro se despachan sobre el
  ...
  - Es una LISTA DE MENSAJES PASIVA, no un sistema de progreso/porcentaje — es
...
patron de barra de progreso modal con callback — se le pasa un delegate un
...
background, patron comun en todo el codebase para escaneos de tamaño, imports,
...
IDialogsFactory/GlobalProgressOptions viven en PlayniteSDK/IDialogsFactory.cs —
no se expandieron del todo en esta pasada porque la tarea solo pedia "lo
suficiente para conocer el patron."

--------------------------------------------------------------------------------
28.16 HALLAZGOS SORPRENDENTES — SEGUNDA PASADA
...
1. El tracking de playtime es 100% por POLLING, nunca por evento. A pesar de
   ...
   EnableRaisingEvents), Playnite nunca lo usa. Cada modo de tracking — incluso
   OriginalProcess, que trackea un unico PID conocido — corre con un loop
...
2. CheckSetupCommand/CheckExecutionCommand NO "chequean" ni "verifican" nada —
...
3. IsInstalled NUNCA se re-verifica activamente por el nucleo de Playnite (no
   ...
   — se setea una vez en un flujo manual de "agregar juego", se cambia por un
...
5. En CADA arranque de la app, el cargador de LiteDB pone en cero
   ...
   TODOS los juegos (transformacion del constructor de GamesCollection) —
   ...
   sesion interrumpida — simplemente se descarta en silencio.
6. safestart.flag suena relacionado a playtime/sesiones por el nombre pero no
   lo esta — es el protector anti-crash-loop del propio Playnite para activar
...
7. Las apps UWP (Windows Store) reciben manejo a medida: lanzar via
   ...
   que el mismo lanzo — busca el WorkDir real de la app via
   Programs.GetUWPApps() y cambia a tracking por directorio en su lugar.
8. NO HAY mecanismo generico integrado de install/uninstall — GenericPlayController
...
9. CompletionStatusSettings se guarda en la base de datos LiteDB de juegos (como
   ...
   donde vive la mayoria de los demas grupos de settings — una ubicacion de
...
10. PlayCount se incrementa AL INICIAR el juego, no al pararlo (adentro de
    ...
    minimo de sesion — un lanzamiento accidental y cierre inmediato igual
    cuenta como "jugado" e incrementa PlayCount.

--------------------------------------------------------------------------------
28.17 QUE SIGUE — ESTO ES REFERENCIA, NO DISEÑO DE BRIDGE
...
en vez de depender de que cada fuente lo provea (Playnite no lo tiene — es un
...
importa — el patron GameId+fuente para dedupe, el orden de scripts, el patron
de tracking por polling, la semantica de filtros AND/OR) es un paso de diseño
aparte, todavia no hecho.

--------------------------------------------------------------------------------
28.18 BOOTSTRAP REAL DE LA APLICACION (Desktop)

ProgramEntry.Main — Playnite.DesktopApp/ProgramEntry.cs lineas 18-91, en orden:
  ...
  3. FileSystem.CreateDirectory(PlaynitePaths.JitProfilesPath), despues
     ...
     StartProfile("desktop") — mecanismo nativo de .NET de perfil de arranque
     JIT, no algo custom.
...
PlayniteApplication y DesktopApplication — todo es composicion manual con new.
...
El composition root real esta partido en dos lugares:
  - El constructor protegido de PlayniteApplication (Playnite/App/
    PlayniteApplication.cs lineas 92-442) — corre ANTES de que exista ningun
  ...
  - DesktopApplication.InstantiateApp() (Playnite.DesktopApp/
    DesktopApplication.cs lineas 145-172) — llamado desde
    ...
    hacia abajo — inyeccion por constructor de manual de libro de texto, sin
...
INSTANCIA UNICA — real y en dos capas:
  1. MUTEX con nombre "PlayniteInstaceMutex" (const instanceMuxet,
     ...
     instancia ya lo tiene, el proceso nuevo NO crea su propia ventana — se
     vuelve un cliente.
...
(enum ApplicationMode — Desktop/Fullscreen, LOS TEMAS SON ESPECIFICOS DE MODO,
...
Chequeo de compatibilidad de version de API — dos copias independientes de
...
MECANISMO DE SUSTITUCION DE ResourceDictionary (ThemeManager.ApplyTheme) — la
parte interesante/sorprendente:
  ...
  2. Enumera app.Resources.MergedDictionaries, encuentra cada diccionario cuyo
     ...
     — estos son los nombres de archivo xaml "aceptables" que un tema custom
  ...
  3. Pre-carga cada uno de esos archivos desde la carpeta del tema custom (si
     ...
     recursos en vivo — aborta con AddonLoadError.Uknown si falla.
  ...
  6. LOS VUELVE A AGREGAR EN UN ORDEN INTERCALADO ESPECIFICO: por cada archivo
     ...
     — si el tema provee un override para esa misma ruta relativa — carga y
  ...
  recursos (ej. imagenes) — {markup:ThemeFile Images\AlgunIcono.png} — resuelve
...
LibraryGetGamesArgs lleva SOLO un cancellation token — sin callback de
...
Rama de HasCustomizedGameImport — consumida en
...
PlaytimeImportMode) (ya documentado en S28.2):
  if (library.Properties?.HasCustomizedGameImport == true) {
    ...
    // que EL MISMO ya persistio/preparo — controla toda la logica de
...
ProcessStoreDownload de S28.3 — LibraryPlugin.GetMetadataDownloader() devuelve
uno de estos (o null), y los callers hacen provider.GetMetadata(game) y
despues lo disponen.

--------------------------------------------------------------------------------
28.21 EDICION MULTIPLE — MITAD DE ESCRITURA (GameEditViewModel)
...
como referencia de partida pero NO se diffea contra el en el guardado — ver
abajo.
...
ALGORITMO DE GUARDADO/APLICACION — ConfirmDialog(bool alreadyClosing) (loop de
escritura empieza linea 662):
  ...
  2. Para cada campo de lista seleccionable con Use<Categoria>Changes == true
     ...
     CompletionStatus), llama AddNewItemsToDb/AddNewItemToDb primero — asi
     nombres de tag/genero/etc. nuevos tipeados en la UI de edicion se
     persisten en la DB ANTES de asignarse a los juegos.
  3. gamesToUpdate = IsMultiGameEdit ? Games : new List<Game> { Game } — LOS
  ...
  4. database.Games.BeginBufferUpdate(), despues RECORRE TODOS los
     ...
     es false quedan COMPLETAMENTE INTACTOS en cada juego — confirma que es
  ...
  5. Para los campos multi-select de lista de ids, NO simplemente asigna
     EditingGame.<Ids> — llama un helper local consolidateIds(selectionList,
     originalIds): toma los ids totalmente seleccionados de la lista, mas —
     para items que quedaron en el estado tri-estado
     indeterminado/mixto (Selected == null) — vuelve a agregar los ids de esa
...
Confirmado presente — una feature completa de backup/restore, pero NO hay una
...
Archivo: Playnite/Backup.cs.
  ...
  Disparador de auto-backup: PlayniteSettings.ShouldDataBackupOnStartup() —
  ...
  PlayniteApplication — escribe un JSON BackupActionFile y setea CmdLine.Backup
...
Disparador — DesktopApplication.ProcessStartupWizard(), llamado desde
...
NavigateNext():
  ...
  2. ProviderSelect -> ProviderConfig: por cada biblioteca marcada mas cada
     ...
     lib.GetSettings(true) } — o sea reusa la UI de settings de primer
  ...
  3. ProviderConfig: itera los selectedPlugins juntados uno a la vez
     (SetPluginConfiguration — llama plugin.Settings.BeginEdit() y bindea el
...
propio login/settings, y deshabilitar el resto — NO toca eleccion de tema, ni
...
--------------------------------------------------------------------------------
28.24 HALLAZGOS SORPRENDENTES — TERCERA PASADA

1. LibraryGetGamesArgs no tiene callback de progreso — solo CancelToken.
   ...
   para su primera fuente, necesita su propio mecanismo — el contrato real de
...
2. Casi todo el contrato de LibraryPlugin es opcional (virtual con defaults
   ...
   una interfaz rica — un plugin que devuelve GetGames() => [] es un
...
3. Los flags Use*Changes de edicion multiple son LATCHES DE TOQUE DE UI en
   ...
   siempre durante esa sesion de dialogo — no hay chequeo de "el valor
   ...
   auto-detectan" para edicion masiva — el producto real depende de flags de
...
5. La deteccion de instancia unica desempata por PID, no por reloj de pared:
   ...
   simplemente "sobrevive el de PID mas alto" — una heuristica que se ve
...
6. La aplicacion de tema es RECARGA DESTRUCTIVA DE TODO, no merge
   incremental — ApplyTheme quita y reconstruye TODA la cadena de
...
8. Se encontro un archivo suelto Documentacion_Reescritura_Playnite.txt (y
   ...
   español que referencian HasCustomizedGameImport — aparentemente
   ...
   deberia ser codigo fuente prístino de Playnite upstream — vale la pena que
...
de la app (sin contenedor de DI — composicion manual por constructor, dato
importante para el diseño de Bridge.App), sistema de temas (deferido a Fase 7
...
siendo REFERENCIA, no diseño de Bridge — adaptar esto a las clases y flujos
...
es el siguiente paso de diseño, todavia no hecho.

--------------------------------------------------------------------------------
28.26 IGDB Y STEAM — VERIFICADO CONTRA EL REPO DE EXTENSIONES OFICIAL
(agregado 2026-08-05, tercera sesion, fuente:
D:\Proyectos\PlayniteExtensions-master\PlayniteExtensions-master, el repo
real de extensiones de Playnite — a diferencia de S28.1-28.25 que analizaron
el nucleo, esto analiza dos addons oficiales concretos porque Bridge los
necesita ahora: IGDBMetadata y SteamLibrary)

IGDB — Playnite NO le pega directo a IGDB desde el cliente:
  ...
  archivo — confirmado por grep, no hay ni una linea de client_secret/token/
  ...
  hace de proxy — el usuario final nunca ve ni carga un Client ID/Secret.
  ...
  usuario sigue siendo valida — es la alternativa sin servidor propio, mas
...
STEAM — deteccion de juegos instalados, 100% archivos locales, sin red:
  ...
  2. GetLibraryFolders() (SteamLocalService.cs linea 341): arranca con
     ...
     NO json — se parsea con la clase KeyValue del paquete NuGet SteamKit2)
  ...
  4. GetInstalledGameFromFile(path) (linea 72): parsea cada .acf con el mismo
     ...
     FullyInstalled o se descarta — hay un enum AppStateFlags completo con
