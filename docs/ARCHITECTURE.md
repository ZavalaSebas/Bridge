...
In the multi-provider chain, Steam-imported games get a guaranteed appid-direct lookup first, then fall back to IGDB search. Non-Steam games try IGDB first, then Steam search by name — **except ROMs**, which are `IgdbFirst` only (Worker → Playnite → user IGDB, never Steam) because DAT already guarantees the title and a Steam hit for a ROM is impossible; `Risk of Rain` etc. are external (`SteamFirst`) not ROM hacks. On startup, `DownloadMissingSteamMetadataAsync` fire-and-forget fetches metadata for all Steam games without a description. **Update (2026-08-30):** removed `GamesView.DeferRefresh()` across `await` (view stayed deferred on initial load → empty list / `Collection was modified`), fixed `OrganizeRomsInFolderAsync` `Games.ToList()` snapshot and removed `IsNetworkAvailable()` early-return that made first open skip sync when Wi-Fi was still connecting — `Refresh` worked because network was up later; startup now always tries HTTP and lets it fail gracefully.
