...
The hero cross-fade (`FadeImage.xaml.cs`, `CoverByWidth`) animates the outgoing tall frame's bottom fade in sync with the incoming short frame (`AnimateOutgoingTallFade` + animated `ApplyShortFrameFadeTo`), so switching from a tall background (e.g. a 16:9 screenshot) to a shorter one no longer snaps the blur edge when opacity reaches zero.
